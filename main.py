...
def fetch_band_page(band):
    ...
    try:
        ...
        if band_page_response.status_code == 200:
            ...
            print(f"Band: {band_name}, Country: {country}, Genre: {genre}, Status: {status}, Photo URL: {photo_url}")
...
def save_to_csv(bands):
    ...
    df.to_csv("metal_bands.csv", mode="a", header=not os.path.exists("metal_bands.csv"), index=False)
